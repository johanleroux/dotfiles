...
CMDLINE_ARGS="KERNEL_CMDLINE[default]+=\"quiet splash\""
if echo "$CMDLINE" | grep -q "quiet splash"; then
  CMDLINE_ARGS=""
fi
echo "$CMDLINE"
echo "$CMDLINE_ARGS"
...
if type system-limine-backup &>/dev/null; then
  system-limine-backup
fi
